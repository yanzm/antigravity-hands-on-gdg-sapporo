...
<span style="color:salmon"><b>※ このドキュメントの内容は 2026/04/24 時点の情報です。Antigravity は活発にアップデートされているため、最新の情報は公式ドキュメント ( https://antigravity.google/docs ) を確認してください。</b></span>
...
- 簡単な修正は Plan を無効にして即実行させる（計画フェーズを省略してクォータを節約）
...
会話を開始する際、入力欄の **+** ボタンのメニューから **Plan** を有効にできます。

- **Plan を有効** : エージェントが Implementation Plan（実装計画）を作成し、ユーザーが承認してから実装に入ります。詳細な調査や複雑なタスクに向いています
- **Plan を無効** : 計画フェーズを省略して即実装に入ります。簡単な修正やクォータ節約に向いています

> **Note** : 以前のバージョンでは「Planning」「Fast」という2つの会話モードを選択する UI でしたが、現在は + メニュー内の Plan トグルに変わりました。
...
- **Implementation Plan** : 変更の設計と技術詳細。Plan を有効にした会話で、実装前にユーザーがレビューする対象として生成されます
- **Task List** : エージェントが実行した作業の一覧。実装完了後に Walkthrough と一緒に表示されます
